...
def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hqrf:e:", ["help", "quite", "recursive", "file=",
                                                             "expression="])
        ...
        if(recursive):
            for dirname, dirnames, filenames in os.walk(filePath):
                ...
                for filename in filenames:
                    ...
                    api_modded.main(os.path.join(dirname, filename), regExpr, quite)
        else:
            for filename in os.listdir(filePath):
                if filename.endswith('.png') or filename.endswith('.jpg') or filename.endswith('.gif') or filename.endswith('.tif'):
                    print(filename)
                    api_modded.main(filePath + '/' + filename, regExpr, quite)


    elif os.path.isfile(filePath):
        api_modded.main(filePath,regExpr,quite)
